#!/bin/bash
brew install --cask alacritty alt-tab betterdisplay brave-browser karabiner-elements keepassxc linearmouse maccy rectangle visual-studio-code
brew install --formula asdf bash fd fzf git git-delta git-extras git-gui gnu-time jq less moreutils ncdu neovim pandoc ripgrep grep rlwrap scc sd tmux zoxide watch coreutils difftastic
...
# this is because I can stand latency in the tab key but I can't
# stand latency on the escape key whatsoever
...
# brave ctrl-l ctrl-h tabs: https://superuser.com/a/1260437
